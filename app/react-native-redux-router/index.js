...
import {PUSH, REPLACE, POP, DISMISS, RESET} from './actions'
...
class RouterComponent extends React.Component {
  constructor(props){
    ...
    let RouterActions = props.actions || Actions;
      ...
      if (child.type.prototype.className() == "Schema") {
        ...
        if (!(RouterActions[name])) {
          RouterActions[name] = function (data) {
            ...
            let args = {name: name, data:data};
            let action = child.props.type || 'push';
        ...
        if (!(RouterActions[name])) {
          RouterActions[name] = function(data){
            let props = self.extend({}, self.props);
  ...
  onChange(page){
    if (page.mode==PUSH || page.mode==REPLACE){
      let route = this.routes[page.currentRoute];
      if (!route){
      ...
      if (route.schema=='popup'){
        let element = React.createElement(route.component, Object.assign({}, route, page.data, {dispatch: this.props.dispatch, routes:this.routerActions}));
    ...
    if (page.mode==POP){
      ...
      let num = page.num || (routes.length - page.routes.length);
  ...
  getRoute(route, data) {
    ...
    let schema = this.schemas[route.schema || 'default'] || {};
    let sceneConfig = route.sceneConfig || schema.sceneConfig || Animations.None;
    let NavBar = route.navBar || schema.navBar;
    let Footer = route.footer || schema.footer;
...
export const Router = connect(state => state.routerReducer)(RouterComponent)
